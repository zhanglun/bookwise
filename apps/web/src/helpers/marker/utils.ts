// https://github.com/DLillard0/canvas-highlighter/blob/main/src/utils/utils.ts
...
export function getTextNodeRects(
  ...
  const range = (win || window).document.createRange();
...
export function getCharRect(node: Text, offset: number, win: Window) {
  const range = (win || Window).document.createRange();
